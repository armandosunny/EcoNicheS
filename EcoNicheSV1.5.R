...
ui <- dashboardPage(
  dashboardHeader(title = 'EcoNicheS V. 1.5.0',
                  ...
                  column(width = 4,
                         ...
                         box(
                           ...
                           selectInput("modelSelection", div("Single Models", tags$i(class = "fas fa-question-circle", "data-toggle" = "tooltip", "title" = "Choose the models or algorithms used during the analysis and evaluation. These are the single models functions, for more information go to the User Manual and/or visit the biomod2 vignette on github.")),
                                       choices = c('CTA', 'FDA', 'GAM', 'GBM', 'GLM', 'MARS', 'MAXENT', 'MAXNET', 'RF', 'RFd', 'SRE', 'XGBOOST'),
                                       multiple = TRUE),
                           selectInput("strategy_Selection", div("Select Strategy", tags$i(class = "fas fa-question-circle", "data-toggle" = "tooltip", "title" = "Strategy used during the calibration and validation of the models, for more information go to the User Manual and/or visit the biomod2 vignette on github.")),
                                       choices = c('random','kfold','block','strat','env','user.defined'),
                                       multiple = FALSE),
                                       conditionalPanel(condition = "input.strategy_Selection == 'kfold' || input.strategy_Selection == 'strat' || input.strategy_Selection == 'env'",
                                        numericInput("cvKbut", "Number of partitions:", value = 2, min = 2, max = 20)),
                           selectInput("metricSelection", div("Select Evaluation Metrics", tags$i(class = "fas fa-question-circle", "data-toggle" = "tooltip", "title" = "Model performance and accuracy evaluation metrics, for more information go to the User Manual and/or visit the biomod2 vignette on github.")),
                                       choices = c('TSS', 'KAPPA', 'ACCURACY', 'AUCroc', 'BOYCE'), # ROC ed
                                       multiple = TRUE),
                           ...
                           selectInput("ensembleMetrics", div("Select Ensemble Metrics", tags$i(class = "fas fa-question-circle", "data-toggle" = "tooltip", "title" = "Adjust the parameters for Ecological Niche Modeling.")),
                                       choices = c('TSS', 'KAPPA', 'ACCURACY', 'AUCroc', 'BOYCE'), # ROC ed
                                       multiple = TRUE),
...
# ====================================
  ...
  req(input$ensembleMetrics)

  lapply(input$ensembleMetrics, function(metric) {

    sliderInput(
      inputId = paste0("threshold_", metric),
      label = paste("Threshold for", metric),
      min = 0,
      max = 1,
      value = 0.4,
      step = 0.1
        ...
        withProgress(message = 'Running selected models...', value = 0, {
          ...
          for (i in 1:total_iterationsbio) {
            ...
            if (!is.null(data()) && !is.null(layers())) {
              ...
              cv_k <- if (input$strategy_Selection %in% c("kfold", "strat", "env")) {
                  input$cvKbut
                    } else {
                      NULL
                      }
              ...
              incProgress(1/10, detail = "Running selected models...Projection")
              ...
              incProgress(1/10, detail = "Running selected models...Ensemble")
              # Model ensemble models

              thresholds <- sapply(input$ensembleMetrics,
                      function(metric) input[[paste0("threshold_", metric)]]
                                                )
              ...
              myBiomodEM <- BIOMOD_EnsembleModeling(bm.mod = MyBiomodModelOut,
                                                    ...
                                                    metric.select = input$ensembleMetrics,
                                                    metric.select.thresh = thresholds,
                                                    metric.eval = input$evalMetrics, 
                                                    var.import = 3,
                                                    seed.val = 42)
              ...
              output$responseCurvesBivariatePlot <- renderPlot({
                mods <- get_built_models(MyBiomodModelOut, run = 'RUN2')
